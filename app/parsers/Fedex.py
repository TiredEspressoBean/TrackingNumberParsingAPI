...
class FedEx(BaseParser):
    def __init__(self):
        ...
        self.patterns = {
            ...
            "FedEx SmartPost": [
                ...
                partial(self.calculate_checksum_mod10, evens_multiplier=3, odds_multiplier=1),
            ],
            "FedEx Ground": [
                r'(?P<SerialNumber>[0-9]\s*){14}(?P<CheckDigit>[0-9])',
                partial(self.calculate_checksum_mod10, evens_multiplier=1, odds_multiplier=3),
            ],
    ...
    def validate(self, tracking_number):
        ...
        :return: Bool of whether the string is valid as a FedEx tracking number or not.
        """
        for pattern_name, (regex, validation_func) in self.patterns.items():
            ...
            if match:
                ...
                valid = False
                if pattern_name == "FedEx Express(12)" or pattern_name == "FedEx Express(34)" or pattern_name == "FedEx Ground GSN":
                    valid = validation_func(serial_number=serial_number, check_digit=check_digit)
                else:
                    valid = validation_func(check_digit=int(check_digit), digits=[int(x) for x in tracking_number])
                if valid:
                    return True
